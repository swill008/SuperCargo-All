...
function ChromeButton({ onClick, icon, label, compact, active }: { onClick: () => void; icon: React.ReactNode; label: string; compact?: boolean; active?: boolean }): React.ReactElement {
  return (
    <Btn onClick={onClick} title={compact ? label : undefined} style={{ display: 'flex', alignItems: 'center', gap: 8, border: `1px solid ${active ? C.acc : C.lineStrong}`, background: active ? C.acc : 'transparent', color: active ? '#111' : C.dim, fontFamily: F.display, fontSize: 12, fontWeight: 600, letterSpacing: '0.14em', padding: compact ? '8px 10px' : '8px 14px', cursor: 'pointer' }} hoverStyle={{ border: `1px solid ${C.acc}`, color: active ? '#111' : C.text }}>
      {icon}
      {!compact && label}
    </Btn>
  )
